...
function reDraw(context, data, canEdit,isOld)
{
    ...
    /*------custom group data view end---------*/
        ...
        if(isOld){
            ...
            var retObj;
            var dataDiff = getDifference(prevData,cleanData);
            console.log(dataDiff)
            $('#data-table-new td[data-key-new]').html('');
            context.meld($('#data_after'), cleanData, {
            selector: function(k) {
                k = k.replace(cleanRe, cleanReplacement);
                for (var chk_key in dataDiff) {
                    if (k == chk_key) {
                        console.log("ok")
                        $('[data-key-new="' + k + '"]').parent().addClass("cell-color");
                    }
                }
...
function createTable(canEdit)
{
    ...
    var table = $('<table id="data-table" class="table table-bordered"></table');
    var tHead = $('<thead><tr><th width="12%">' + gettext("Question") + '</th><th width="12%">' + gettext("Response") + '</th></tr></thead>');
...
function createNewTable(canEdit)
{
    ...
    var table = $('<table id="data-table-new" class="table table-bordered"></table');
    var tHead = $('<thead><tr><th width="12%">' + gettext("Question") + '</th><th width="12%">' + gettext("Response") + '</th></tr></thead>');
    var tBody = $('<tbody></tbody>');
    var key;
    var GROUP_CREATED = false;
