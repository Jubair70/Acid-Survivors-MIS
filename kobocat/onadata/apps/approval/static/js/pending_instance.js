...
function createTable(canEdit)
{
    ...
    <form action="" onsubmit="return addOrEditNote()" method="post" name="notesform" id="notesform"> \
        ...
        <button type="submit" id="approve" class="btn btn-small btn-primary" >' + gettext("Approve ") + '</button> \
	    <button type="submit" id="reject" class="btn btn-small btn-primary" >' + gettext("Reject ") + '</button> \
        </div>\
    </form> \
    <div id="notes-section"></div></div>';
    console.log( window.is_approved_or_reject);
    if(!window.is_approved_or_reject){
    ...
    if(data)
    {
        ...
        if(notes.length > 0){
            for(note in notes){
                ...
                notesHTML += '<tr><td>' + n['note'] + '</td><td>' + n['date_modified'] + '</td><td>' + n['user_id'] + '</td><td>'
