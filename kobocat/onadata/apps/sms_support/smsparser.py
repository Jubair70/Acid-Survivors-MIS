...
from onadata.apps.sms_support.tools import SMS_API_ERROR, SMS_PARSING_ERROR,\
    ...
    is_last
from onadata.libs.utils.logger_tools import dict2xform_sms
...
def parse_sms_text(xform, identity, text):
    ...
    print 'Himel: separator: '+separator
    try:
    ...
    for expected_group in json_survey.get('children', [{}]):
        ...
        print 'Himel:: parser:: group_id:: '+ str(group_id)
...
def process_incoming_sms(username, incomings,
                           id_string=None):
    ...
    def process_incoming(incoming, id_string):
        ...
        json_submission, medias_submission, notes = parse_sms_text(xform,
                                                                   identity,
                                                                   text)
        print 'Himel: json Submission: '+str(json_submission)
        # retrieve sms_response if exist in the form.
        json_survey = json.loads(xform.json)
        print 'himel json_survey: '+ str(json_survey)
        ...
        print 'himel meta_groups: '+ str(meta_groups)
       #belog code is off for testing purpose.. himel 
       ...
       #     return
        ...
        print 'Himel:: submitted fields'+str(submitted_fields)
        ...
        for field in required_fields:
            if not submitted_fields.get(field):
                ...
                return
        form_tag = None
        if json_survey.get('name'):
            form_tag = json_survey.get('name')
        # convert dict object into an XForm string
        xml_submission = dict2xform_sms(jsform=json_submission,
                                    form_id=xform.id_string,form_name=form_tag)
        #print 'himel::xml_submission:: '+ xml_submission
