...
def instance(request, username, id_string):
    ...
    xform, is_owner, can_edit, can_view,can_submit,can_delete = get_form_permissions(id_string,username,str(request.user))

    #qry="select ns_label,ns_value from vwapprovaldef where role_id in(select role_id::text from vwusermodule_userrolemap where auth_user_id="+str(request.user.id)+")"
